...
class WallDetector(Node):
    ...
    def on_scan(self, scan_msg):
        ...
        # Convert the scan points into the rectangular frame
        xs = np.cos(thetas) * rs
        ys = np.sin(thetas) * rs
        xy = np.array([xs, ys])
        self.current_scan_points = xy

        # Detect walls using the hough transform
        ...
        # Calculate out some concrete points along the wall to publish as markers
    ...
    def plot_hough(self, request, response):
        # ROS Service callback for plotting the results of the hough transform in Matplotlib
    ...
    def hough_tform(self):
        ...
        # Generate the heatmap of the parameter space: "vote" for the maximal theta and rho
        ...
        # Choose rho and theta based on the highest point in the heatmap
